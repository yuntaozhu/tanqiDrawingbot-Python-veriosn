...
async def get_system_status():
    """获取系统运行状态"""
    return {
        "uptime": time.time() - start_time_app,
        "caches": {
        ...
        "pending_jobs": len(print_jobs),
        "env_check": {
            "replicate": bool(os.getenv("REPLICATE_API_TOKEN")),
            "ideogram": bool(os.getenv("IDEOGRAM_API_KEY")),
            "deepseek": bool(os.getenv("DEEPSEEK_API_KEY")),
            "stt": bool(os.getenv("STT_API_KEY") or os.getenv("SILICONFLOW_API_KEY")),
            "tts": bool(os.getenv("TTS_API_KEY") or os.getenv("SILICONFLOW_API_KEY"))
        }
    }
...
async def get_admin_history():
    """获取最近的生成历史"""
    return sorted(generation_history, key=lambda x: x.get("timestamp", ""), reverse=True)[:50]
...
<body class="p-6">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-3xl font-bold text-slate-800">探奇后台管理 <span class="text-sm font-normal bg-green-100 text-green-700 px-2 py-1 rounded ml-2">运行中</span></h1>
                <p class="text-slate-500">监控、管理与调试 探奇 AI 服务</p>
            </div>
            <button onclick="refreshData()" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                <div class="text-slate-400 text-sm mb-1 uppercase font-semibold">图片缓存</div>
                <div id="image-cache-count" class="text-3xl font-bold text-slate-700">0</div>
            </div>
            <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                <div class="text-slate-400 text-sm mb-1 uppercase font-semibold">语音统计 (STT/TTS)</div>
                <div id="voice-cache-count" class="text-3xl font-bold text-slate-700">0 / 0</div>
            </div>
            <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                <div class="text-slate-400 text-sm mb-1 uppercase font-semibold">生成总数</div>
                <div id="history-count" class="text-3xl font-bold text-slate-700">0</div>
            </div>
            <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                <div class="text-slate-400 text-sm mb-1 uppercase font-semibold">待处理打印</div>
                <div id="pending-jobs" class="text-3xl font-bold text-slate-700">0</div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 space-y-6">
                <!-- Cache Management -->
                <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                    <h2 class="text-xl font-bold mb-4 text-slate-800"><i class="fas fa-tools mr-2"></i> 管理操作</h2>
                    <div class="space-y-3">
                        <button onclick="clearAllCache()" class="w-full bg-red-50 hover:bg-red-100 text-red-600 font-semibold py-3 rounded-lg border border-red-200 transition">
                            <i class="fas fa-trash-alt mr-2"></i> 清理全部缓存
                        </button>
                        <p class="text-xs text-slate-400">清理后，重复的绘图请求将重新调用昂贵的外部 API。</p>
                    </div>
                </div>

                <!-- API Status -->
                <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                    <h2 class="text-xl font-bold mb-4 text-slate-800"><i class="fas fa-key mr-2"></i> API 配置检测</h2>
            ...
            <div class="lg:col-span-2">
                <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                    <div class="p-6 border-b border-slate-100 flex justify-between items-center">
                        <h2 class="text-xl font-bold text-slate-800"><i class="fas fa-history mr-2"></i> 最近绘图历史</h2>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            <thead class="bg-slate-50 text-slate-500 text-sm">
                                <tr>
                                    <th class="px-6 py-3 font-semibold">内容 (Prompt)</th>
                                    <th class="px-6 py-3 font-semibold">生成引擎</th>
                                    <th class="px-6 py-3 font-semibold">预览</th>
                    ...
                    <div id="no-history" class="p-12 text-center text-slate-400 hidden">
                        暂无生成历史
                    </div>
                </div>
            </div>
    ...
    <script>
        async function refreshData() {
            try {
                ...
                document.getElementById('image-cache-count').innerText = status.caches.images;
                document.getElementById('voice-cache-count').innerText = `${status.caches.stt} / ${status.caches.tts}`;
                document.getElementById('history-count').innerText = status.history_count;
                document.getElementById('pending-jobs').innerText = status.pending_jobs;

                // Update Env Status
                let envHtml = '';
                for (const [key, val] of Object.entries(status.env_check)) {
                    envHtml += `
                        <div class="flex items-center justify-between p-2 rounded ${val ? 'bg-green-50' : 'bg-red-50'}">
                            <span class="text-sm font-medium capitalize text-slate-700">${key} API</span>
                            <span class="${val ? 'text-green-600' : 'text-red-600'} text-xs">
                                <i class="fas ${val ? 'fa-check-circle' : 'fa-exclamation-triangle'}"></i> ${val ? '已配置' : '未配置'}
                            </span>
                        </div>
                    `;
                }
                    ...
                    history.forEach(item => {
                        const row = `
                            <tr class="hover:bg-slate-50 transition">
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-slate-800">${item.english_prompt || item.prompt}</div>
                                    <div class="text-xs text-slate-400">${item.prompt}</div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-xs bg-slate-100 px-2 py-1 rounded text-slate-600 uppercase font-bold">${item.engine || 'unknown'}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <img src="${item.original_url}" class="w-12 h-12 rounded shadow-sm hover:scale-150 transition cursor-pointer object-cover">
                                </td>
        ...
        async function clearAllCache() {
            if (!confirm('确定要清除所有图片和语音缓存吗？这会增加后续请求的成本。')) return;
            try {
