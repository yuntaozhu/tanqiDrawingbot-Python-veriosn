...
def convert_audio_to_target_samplerate(audio_bytes: bytes, target_sr: int = 24000) -> Optional[bytes]:
    ...
    This function gracefully degrades:
    - If ffmpeg is unavailable → returns original audio
    - If conversion fails → returns original audio and logs warning
    - If conversion times out → returns original audio and logs warning
    """
    ...
    try:
        ...
        result = subprocess.run([
            'ffmpeg', '-loglevel', 'error',  # Suppress ffmpeg verbose output
            '-i', 'pipe:0',
            '-ar', str(target_sr),           # Set sample rate
            '-c:a', 'libmp3lame',            # MP3 codec
            '-q:a', '4',                     # Quality ~192kbps
            '-f', 'mp3',
            'pipe:1'
        ], 
        input=audio_bytes,
